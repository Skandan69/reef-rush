...
function stepStrike(dt) {
  ...
  for (const f of fishes) {
    ...
    /* a hurt fish makes for the nearest crate it can see */
    if (f.hp < SK_MAX_HP * 0.55 && SK.drops.length) {
      let bd = 1e9, bx = 0, by = 0;
      for (const d of SK.drops) {
        const dd = Math.hypot(d.x - f.x, d.y - f.y);
        if (dd < bd) { bd = dd; bx = d.x; by = d.y; }
      }
      if (bd < 2200) f.ta = Math.atan2(by - f.y, bx - f.x);
    }
  ...
  for (let i = SK.drops.length - 1; i >= 0; i--) {
    ...
    if (playing && Math.hypot(player.x - d.x, player.y - d.y) < player.r + 90) { skTakeDrop(d, i); continue; }
    /* the shoal competes for the same crates - a hurt fish that reaches a
       heart first has genuinely taken it from you */
    let taken = false;
    for (const f of fishes) {
      if (f === player || f.dead || f.kind === "ghost") continue;
      if (Math.hypot(f.x - d.x, f.y - d.y) > f.r + 70) continue;
      if (d.kind === "heart") {
        skArm(f);
        f.hp = Math.min(SK_MAX_HP, f.hp + 30);
        f.hurt = 0.35;
        floatText(f.x, f.y - f.r, "+30", "#ff6f8b");
      }
      taken = true;
      break;
    }
    if (taken) { spark(d.x, d.y, 26, "rgba(255,220,170,1)"); SK.drops.splice(i, 1); }
  }
