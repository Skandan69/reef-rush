...
import { endRaid, readRaidState, startRaid } from "./raid";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      /* Raids. The target is handed out by the server rather than named by the
         client, and what a raid was worth is decided at /end from facts the
         server already holds — see raidrules.js for why. */
      if (url.pathname === "/api/raid/state" && request.method === "GET") {
        return readRaidState(env, (url.searchParams.get("pid") ?? "").slice(0, 64));
      }
      if (url.pathname === "/api/raid/start" && request.method === "POST") {
        let body: unknown;
        try {
          body = await request.json();
        } catch {
        ...
        return startRaid(env, body);
      }
      if (url.pathname === "/api/raid/end" && request.method === "POST") {
        let body: unknown;
        try {
          body = await request.json();
        } catch {
