  ...
  for (let i = globs.length - 1; i >= 0; i--) {
    ...
    for (let j = 0; j < fishes.length; j++) {
      ...
      if (BOSS.on && !BOSS.dead && g.owner === player &&
        ...
        hurtBoss(bossDamageFrom(G.score, 26 * g.mass));
...
function drawCritters() {
  ...
  for (let i = 0; i < critters.length; i++) {
    ...
    if (c.kind === "poseidon" && ART.poseidon) {
      drawSprite(ART.poseidon, k, c.ph, 0.05, 0.03);
    } else if (c.kind === "mermaid" && ART.mermaid) {
      drawSprite(ART.mermaid, k, c.ph, 0.07, 0.05);
...
function bossDamageFrom(pts, base) {
  ...
  return base * (0.35 + Math.min(1.6, pts / BOSS_SOLO));
}
...
function stepBoss(dt) {
  ...
  if (d < 420 + player.r) {
    const dmg = bossDamageFrom(G.score, 90 * dt);
    if (dmg > 0) {
      hurtBoss(dmg);
...
function drawBoss() {
  ...
  if (ART.poseidon) {
    /* the painted sea king, with the aura and the charging trident still
       drawn in code around him */
    const h = k * 3.6, w = h * (ART.poseidon.width / ART.poseidon.height);
    ctx.save();
    ctx.rotate(Math.sin(BOSS.ph * 0.4) * 0.02);
    if (BOSS.hit > 0) ctx.filter = `brightness(${1 + BOSS.hit * 0.6})`;
    ctx.drawImage(ART.poseidon, -w / 2, -h * 0.42, w, h);
    ctx.filter = "none";
    ctx.restore();
